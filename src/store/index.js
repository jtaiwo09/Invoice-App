...
export default createStore({
  ...
  actions: {
    GET_INVOICES: async ({ commit, state }) => {
      const getData = await db.collection("invoices").get();
      getData.forEach((doc) => {
    ...
    UPDATE_INVOICE: async ({ commit, dispatch }, { docId, routeId }) => {
    ...
    DELETE_INVOICE: async ({ commit }, docId) => {
      const getInvoice = db.collection("invoices").doc(docId);
      await getInvoice.delete();
      commit("DELETE_INVOICE", docId);
    },
    UPDATE_STATUS_TO_PAID: async ({ commit }, docId) => {
      const getInvoice = db.collection("invoices").doc(docId);
    ...
    UPDATE_STATUS_TO_PENDING: async ({ commit }, docId) => {
      const getInvoice = db.collection("invoices").doc(docId);
      await getInvoice.update({
        invoicePaid: false,
        invoicePending: true,
